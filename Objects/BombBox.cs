...
public class BombBox : Box
{
    Texture2D Texture { get; set; }
    private int _activeIndex;
    public BombBox(int width, int height, Vector2 originPos, string header, Color bgColor, Color fontColor, Texture2D texture) : base(width, height, originPos, header, bgColor, fontColor)
    {
        Texture = texture;
        _activeIndex = 0;
    }
    ...
    public override void Draw()
    {
        ...
        for (var i = 1; i <= Globals.PlayerState.BombNum; i++)
        {
            var vp = BubbleHelper.SpecialBubbleViewPort[BubbleSpecial.Bomb];
