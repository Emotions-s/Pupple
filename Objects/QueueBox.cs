...
public class QueueBox : Box
{
    ...
    public override void Update()
    {
        ...
        && InputManager.MousePosition.Y <= OriginPos.Y + Globals.GridSize * 2)
        {
            ...
            if (indexX >= 1 && indexX <= PlayerState.BubbleQueueMaxSize)
            {
                _activeIndex = indexX;
    ...
    public override void Draw()
    {
        ...
        for (var i = 1; i <= PlayerState.BubbleQueueMaxSize; i++)
        {
            var vp = Globals.Shooter.BubbleQueue[i] == null ? BubbleHelper.NormalBubbleViewPort[BubbleColor.White] : BubbleHelper.NormalBubbleViewPort[((NormalBubble)Globals.Shooter.BubbleQueue[i]).Color];
